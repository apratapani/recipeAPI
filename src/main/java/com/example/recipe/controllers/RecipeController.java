...
public class RecipeController {
    ...
    @GetMapping("/search/avgRating/{avgRating}")
    public ResponseEntity<?> getRecipesByNameByAvgRating(@PathVariable("avgRating") float avgRating) {
        try {
            return ResponseEntity.ok(recipeService.getAllRecipesMinRating(avgRating));

        } catch (NoSuchRecipeException e) {
         //   return ResponseEntity.status(HttpStatus.NOT_FOUND).body(e.getMessage());
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("At the right place");
        }
    }
